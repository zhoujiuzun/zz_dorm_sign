...
每天允许签到的时间范围：**21:00–22:20**。早于 21:00 服务器拒签。
自动签到的定时器每分钟触发一次覆盖该窗口；每人由 `哈希(openid+日期)` 分到窗口内一个
**专属固定时刻**，仅当分钟命中该时刻才为其签到（同人同天固定、人人错开、每天变化）。

## 失败通知 (Failure Alert)

每分钟那批签到里**只要有失败/异常**，就把该批汇总通过 Server酱 推送给**管理员一人**
（非每个熟人）；全部成功则静默不打扰。配合公开[状态页](#状态页-status-page)构成双重兜底。
OpenID 永久不变、不会失效，故通知主要针对"签到被服务器拒绝"等异常，管理员据此排查
（如时间窗变动、接口变更）。
...
存储于 OSS `history.json`，每次签到追加一条 `{nickname, date, time, status, message}`，**永久保留**。
